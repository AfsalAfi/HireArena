...
class AuthService implements IAuthService {
  ...
  async verifyToken(token: string): Promise<{
    ...
    if (userData.accType === "company") {
      const existingData = await this.companyRepository.findByEmail(
        userData.email
      );
      ...
      if (existingData) {
       ...
       savedUser = await this.companyRepository.create({
        name: userData.name,
  ...
  async loginCompany(email: string, password: string): Promise<IAuthResponse> {
    const user: IUser | null = await this.companyRepository.findByEmail(email);
